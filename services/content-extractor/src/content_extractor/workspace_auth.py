"""Keyless Domain-Wide Delegation helpers for Workspace read APIs."""
...
def build_docs_service() -> Any:
    return build("docs", "v1", credentials=workspace_credentials(DOCS_SCOPES), cache_discovery=False)


def build_sheets_service() -> Any:
    return build("sheets", "v4", credentials=workspace_credentials(SHEETS_SCOPES), cache_discovery=False)
