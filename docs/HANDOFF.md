...
Continue CAPITAL INDEX from HANDOFF.md. Drive Changes fallback, event-ingestor, metadata-loader, policy-engine, content-extractor, drive-governance, and entity-extractor Cloud Run workers are deployed. Controlled /events, /files, /policy_decisions, /extracted_text, /review_queue, /cleanup_queue, and /entity_extractions Firestore writes are verified. Pub/Sub push execution, event->metadata->policy->extraction job chains, metadata-loader Drive API authoritative refetch, content-extractor Docs API read, review_queue handling for empty extraction results, review_queue markdown projection, and Next admin-web `/review` console with Firebase Auth, review action audit trail, Cleanup Queue tab, and Source Files tab are verified. Content-extractor now supports Google Docs, Google Sheets, and Markdown/plain text in code and production image; controlled live extraction wrote `/extracted_text` for real Markdown file `2026-05-27.md` and real Google Sheet `AI_Bridge_Log_Spreadsheet`. Those same live files are also now written to `/files` with authoritative metadata and safe source-quality defaults: source_status=needs_human_review, index_eligible=false, human_block=false. Source Files admin UI can mark selected `/files` active/index_eligible or do_not_index and writes `/source_quality_actions`; it does not mutate Drive. Read flags remain disabled when idle. Policy-engine now carries name/mime_type/web_view_link into policy decisions. Drive Governance is required before broad AI/entity extraction: files need source_status and index_eligible; duplicate/stale/empty/archive/unknown files must not enter AI context or graph as independent evidence; cleanup recommendations require Policy Engine + human approval; hard delete is forbidden. Drive scanner/reconciler local prototype is implemented and has performed a controlled metadata-only scan of Drive root `1No6LMuCpH2T2jmGL7hnFlMto-0gnnHpq`: first 10 inventory files were written to `/files`; metadata-loader revision `capital-metadata-loader-00016-ttk` preserves existing source_status, index_eligible and human_block during refresh. Drive Governance MVP detects empty, duplicate, stale/version, and unknown-project candidates; Cloud Run service `capital-drive-governance` is deployed with WRITE_ENABLED=false and REQUEST_WRITE_ENABLED=true; Cloud Scheduler job `capital-drive-governance-daily` runs daily at 03:17 Asia/Dubai and was manually verified with Cloud Run 200. `/cleanup_queue/cleanup_000000000000000000000001` exists for the empty test doc. Entity-extractor Drive Governance gate, provider-neutral AI extraction contract, Firestore reader/writer boundary, Cloud Run worker, OpenAI Responses API adapter, and Gemini generateContent adapter are implemented and tested: only source_status=active, index_eligible=true, human_block=false can proceed; blocked candidates never call/use AI output; controlled Gemini live extraction for approved `2026-05-27.md` wrote `/entity_extractions/entity_extraction_453f8a24b2af348fba42f04c` with 7 entities and 7 relationships. Secret Manager secret `capital-gemini-api-key` exists with version 1; entity-extractor has secretAccessor only on that secret; AI_PROVIDER_ENABLED=false in production. Admin-web cleanup APIs read /cleanup_queue and write /cleanup_actions but do not mutate Drive. Workers use authenticated ingress=all for Pub/Sub push and WRITE_ENABLED=false when idle; event-ingestor has PUBLISH_METADATA_JOBS=false, metadata-loader has DRIVE_REFETCH_ENABLED=false plus PUBLISH_POLICY_JOBS=false, policy-engine has PUBLISH_EXTRACTION_JOBS=false, content-extractor has DOCS_READ_ENABLED=false plus SHEETS_READ_ENABLED=false plus DRIVE_READ_ENABLED=false plus REVIEW_QUEUE_ENABLED=false, drive-governance has WRITE_ENABLED=false, and entity-extractor has WRITE_ENABLED=false plus REQUEST_WRITE_ENABLED=false plus AI_PROVIDER_ENABLED=false when idle. Next step: turn Drive scanner/reconciler into a Cloud Run scheduled worker, then run governance over the expanded `/files` inventory and only then promote approved files into content/entity extraction. Do not implement Drive mutation or deletion yet.
