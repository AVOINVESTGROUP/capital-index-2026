...
Before a file can influence AI analysis, embeddings, graph extraction, project status, or AI context,
CAPITAL INDEX must assign a source quality state.
...
Only files with all of the following may enter AI analysis:
...
Duplicate, stale, empty, archive-candidate, unknown, and manually blocked files must not be used as
independent evidence in AI context or knowledge graph extraction.

Drive cleanup is recommendation-first:

```text
AI/system may recommend cleanup.
Policy Engine must evaluate cleanup risk.
Human approval is required for archive, move, label, or delete actions.
Hard delete is forbidden in automated flow.
```
...
### 5.3 Drive governance ingestion

Drive governance uses the same metadata foundation as Drive ingestion, but its purpose is source
quality, not content understanding.

Inputs:

```text
Drive metadata
Drive path / parent chain
owners and permissions
file size
mime type
created_time / modified_time
head revision id
extracted text stats when available
folder policy
...
```text
/files/{file_id}.source_status
/files/{file_id}.index_eligible
/cleanup_queue/{cleanup_id}
/cleanup_actions/{action_id}
/file_duplicates/{cluster_id}
```

Governance recommendations are advisory. They do not delete, archive, move, or relabel Drive files
...
### 14.3 `drive-governance-worker`

```text
reads /files metadata and extracted text stats
detects empty files, duplicate candidates, stale candidates, archive candidates
assigns source_status and index_eligible
creates cleanup recommendations
writes /cleanup_queue
writes /file_duplicates for duplicate clusters
never deletes, moves, archives, or relabels Drive files automatically
requires Policy Engine and human approval before any Drive mutation
```

### 14.4 `policy-engine-worker`
...
```text
Firestore approved knowledge graph -> context publication -> AI Gateway / Vault preview
```

The publisher does not send a single Drive link as "knowledge". It builds an
...
Input is limited to policy-approved records:

```text
files.source_status = active
files.index_eligible = true
files.human_block != true
extracted_text.ai_context_allowed = true
entity_extractions.status = extracted
```

The bundle includes owner profile context, source file IDs, multiple evidence
links, claims, entity IDs and relationship IDs. The AI Gateway consumes the
bundle and may request more specific source material through approved evidence
IDs. Claims, entities and relationships generated by AI remain proposals until
human/review-orchestrator approval.

Default write mode is dry-run. Firestore writes require either:

```text
WRITE_ENABLED=true
```

or explicit request-gated writes:
...
Vault output is a preview with protected generated blocks. Manual notes remain
outside generated blocks and are not overwritten.

### 14.17 `review-orchestrator`
...
```json
{
  ...
  "cleanup_recommendation_id": "string|null",
...
```json
{
  ...
  "file_id": "string",
  "project_id": "string|null",
  "source_status": "candidate_duplicate|candidate_stale|candidate_empty|candidate_archive|needs_human_review",
  "reason": "duplicate|near_duplicate|stale|empty|orphaned|version_superseded|unknown_project|temporary_name",
  "recommended_action": "keep_active|mark_duplicate|archive|move_to_review|do_not_index|needs_review",
  "confidence": 0.0,
  "evidence": {
  ...
  "status": "open|approved|rejected|ignored|applied",
  "created_at": "timestamp",
...
### 18.9 `/cleanup_actions/{action_id}`
...
```json
{
  ...
  "file_id": "string",
  "actor_id": "string",
  "actor_type": "human_operator|system",
  "action": "keep_active|mark_duplicate|archive|move_to_review|do_not_index|ignore",
  "previous_source_status": "string",
  "new_source_status": "string",
  "drive_mutation": "none|label|move|archive|delete",
...
6. drive-governance-worker evaluates source quality, duplicate/stale/empty state, and index eligibility.
7. policy-engine applies Drive Labels, folder policy, source registry, and governance state.
8. classifier-worker determines document type, project, business, sensitivity.
9. content-extractor or document-ai-worker extracts content if allowed and index_eligible.
10. chunker-worker creates chunks.
11. embedder-worker creates embeddings if allowed.
12. entity-extractor extracts people, companies, vehicles, money, deadlines, risks.
13. relationship-extractor builds links.
14. dedupe-clusterer assigns canonical cluster.
15. graph-builder updates knowledge graph.
16. validator checks schema, confidence, policy, and source eligibility.
17. vault-writer updates Vault protected blocks.
18. context-publisher updates AI context files.
19. sheets-dashboard-writer updates dashboard.
20. audit-writer writes to BigQuery.
21. cost-controller updates cost state.
22. review-orchestrator creates approval tasks for disputed cases.
```
...
```text
drive-governance-worker
file inventory quality states
empty file detection
duplicate candidate detection
stale/version candidate detection
cleanup_queue
cleanup_actions
index eligibility gate before AI extraction
human-approved archive/move/label workflow
...
6. All AI-eligible files have source_status and index_eligible.
7. Duplicate/stale/empty candidates are excluded from AI context until resolved.
8. Cleanup recommendations require human approval and audit log.
9. SECRET / DO_NOT_INDEX are not read by AI.
10. All AI calls are audited.
11. All workers are idempotent.
12. All workers use leases for concurrency control.
13. Backpressure and circuit breaker are active.
14. DLQ works.
15. Review queue works across Telegram + backup channel.
16. Vault updates only through protected blocks.
17. Vault backup before write works.
18. AI context files generate automatically.
19. Dedupe works through canonical clusters.
20. Knowledge graph contains project/entity/person/file relationships.
21. Cost attribution works by project and model.
22. Manual approval is required for sensitive/destructive actions.
23. Existing GCP/Firebase projects are connected through registry.
24. Migration from current Sheet/Vault/context files is validated.
25. DR drill has passed at least once.
26. Testing fixtures and synthetic events exist.
27. AI quality drift monitoring is active.
```
